...
class AnalysisParameters:
    ...
    def __str__(self):
        ...
        r += '\n{0:<30s} {1:s}'.format(
            'main_harmonic',
            '{0:d} ({1:s})'.format(
                self.main_multipole_harmonic,
                get_harmonic_label(self.main_multipole_harmonic),
            ),
        )
        r += '\n{0:<30s} {1:s}'.format(
            'main_harmonic_is_skew', str(self.main_multipole_is_skew)
        )
        r += '\n{0:<30s} {1:f}'.format(
            'reference_radius[mm]', 1000 * self.ref_radius
        )
...
class RadialRotatingCoil:
    def __init__(self, lines):
        for line in lines:
            ...
            first = words[0]
            if first[0] == '#':
                if len(words) > 3 and words[3] in (
                    'Armazenados(V.s)',
                    'Stored(V.s)',
                ):
                    self.data_conversion_factor = float(
                        words[4].replace('[', '').replace(']', '')
                    )
                continue
                ...
                'raio_interno_bobina_princip(m)',
                'main_coil_internal_radius(m)',
            ):
                self.inner_radius = float(words[1])
            elif first in (
                'raio_externo_bobina_princip(m)',
    ...
    def __str__(self):
        ...
        r += '\n{0:<30s} {1:s}'.format(
            'rotation_direction', self.rotation_wise
        )
        r += '\n{0:<30s} {1:f}'.format(
            'rotation_velocity[rps]', self.rotation_velocity
        )
        r += '\n{0:<30s} {1:f}'.format(
            'rotation_acceleration[rps^2]', self.rotation_acceleration
        )
...
class Measurement:
    def __init__(self, filename):
        ...
        for line in raw_data_txt:
            raw_data[:, idx] = [
                self.rotating_coil.data_conversion_factor * float(word)
        ...
        raw_data_corrected = _numpy.cumsum(
            _numpy.roll(linear_data_corrected, 0)
        )
        raw_data_corrected = _numpy.reshape(
            raw_data_corrected, (self.nr_turns, -1)
        )
    ...
    def __str__(self):
        ...
        r += '\n\n--- measurement ---\n\n'
        r += '{0:<30s} {1:s}'.format(
            'data_filename', _os.path.basename(self.filename)
        )
        r += '\n{0:<30s} {1:s}'.format('time_stamp', self.timestamp)
        r += '\n{0:<30s} {1:+.3f}'.format(
            'main_coil_current_avg[A]', self.main_coil_current_avg
        )
        r += '\n{0:<30s} {1:.3f}'.format(
            'main_coil_current_std[A]', self.main_coil_current_std
        )
...
class Analysis:
    ...
    def _run_analysis(self):
        if self.measurement.rotating_coil.type in (
            'Bobina Radial',
            'Radial Coil',
        ):
    ...
    def _calc_multipoles_for_radial_rotcoil(self):
        ...
        for i in range(nr_turns):
            flux = flux_data[i, :]
            angle = _numpy.linspace(
                2 * _math.pi / len(flux), 2 * _math.pi, len(flux)
            )
            ...
            for harmonic in harmonics:
                ...
                b_n = (
                    +n * integ_cos / _math.pi / nr_coil_turns / (r2**n - r1**n)
                )
                a_n = (
                    -n * integ_sin / _math.pi / nr_coil_turns / (r2**n - r1**n)
                )
    ...
    def _calc_relative_multipoles(self):
        ...
        if main_is_skew:
            main_multipole_at_ref_radius = self.polynom_a[:, idx] * (
                ref_radius ** (main_harmonic - 1)
            )
        else:
            main_multipole_at_ref_radius = self.polynom_b[:, idx] * (
                ref_radius ** (main_harmonic - 1)
            )
        self.polynom_a_relative = 0 * self.polynom_a
        self.polynom_b_relative = 0 * self.polynom_b
        for i in range(len(harmonics)):
            polynom_a_at_rotcoil_radius = self.polynom_a[:, i] * (
                ref_radius ** (harmonics[i] - 1)
            )
            polynom_b_at_rotcoil_radius = self.polynom_b[:, i] * (
                ref_radius ** (harmonics[i] - 1)
            )
            self.polynom_a_relative[:, i] = (
                polynom_a_at_rotcoil_radius / main_multipole_at_ref_radius
            )
            self.polynom_b_relative[:, i] = (
                polynom_b_at_rotcoil_radius / main_multipole_at_ref_radius
            )

    def _calc_statistics(self):
        self.polynom_a_relative_avg = _numpy.mean(
            self.polynom_a_relative, axis=0
        )
        self.polynom_a_relative_std = _numpy.std(
            self.polynom_a_relative, axis=0
        )
        self.polynom_b_relative_avg = _numpy.mean(
            self.polynom_b_relative, axis=0
        )
        self.polynom_b_relative_std = _numpy.std(
            self.polynom_b_relative, axis=0
        )
    ...
    def __str__(self):
        ...
        r += 'integrated multipoles relative to main multipole at r0 = {0:.1f} mm\n'.format(
            1000 * self.parameters.ref_radius
        )
        ...
        for i in range(len(self.parameters.harmonics)):
            ...
            r += '{0:02d}  {1:+.4e}  {2:.4e}   {3:+.4e}  {4:.4e}    {5:<10s}\n'.format(
                h,
                self.polynom_b_relative_avg[i],
                self.polynom_b_relative_std[i],
                self.polynom_a_relative_avg[i],
                self.polynom_a_relative_std[i],
                get_harmonic_label(h),
            )
        r += '\n'
        r += 'integrated multipoles\n'
        r += '{0:2s}  {1:<11s}  {2:<11s}  {3:<11s}  {4:<11s}    {5:<10s}    units\n'.format(
            'n', '  avg (Nn)', ' std (Nn)', '  avg (Sn)', ' std (Sn)', 'label'
        )
        for i in range(len(self.parameters.harmonics)):
            h = self.parameters.harmonics[i]
            r += '{0:02d}  {1:+.4e}  {2:.4e}   {3:+.4e}  {4:.4e}    {5:<10s}    {6:s}\n'.format(
                h,
                self.polynom_b_avg[i],
                self.polynom_b_std[i],
                self.polynom_a_avg[i],
                self.polynom_a_std[i],
                get_harmonic_label(h),
                get_units(h),
            )
        r += '\nlegend:'
...
def run_analysis(parms, fnames, print_flag=True):
    ...
    for i in range(len(fnames)):
        ...
        *first, fname = _os.path.split(fnames[i])
        if print_flag:
            print(
                '{3:02d} - {0}, current: {1:+9.4f} +/- {2:.4f}'.format(
                    fname,
                    a.measurement.main_coil_current_avg,
                    a.measurement.main_coil_current_std,
                    i,
                )
            )
        analysis.append(a)
...
def get_maximum_main_multipole(current_data_set, parms, current_threshold=0.0):
    ...
    selection = select_ramp_up(
        current_data_set, current_threshold=current_threshold
    )
...
def get_multipole_from_data_set(
    current_data_set, parms, h, mtype='normal', relative=False
):
...
def find_current(
    ...
    for d1 in meas_data_set:
        ...
        c, s = get_multipole_from_data_set(
            d2,
            parms,
            parms.main_multipole_harmonic,
            mtype='normal',
            relative=False,
        )
        if _numpy.all(_numpy.diff(s) > 0):
            tc = _numpy.interp(
                multi, s, c, left=float('nan'), right=float('nan')
            )
        else:
            tc = _numpy.interp(
                -multi,
...
def plot_relative_multipoles(
    ...
    for d1 in magnet_data_set:
        ...
        for d2 in d1:
            ...
            cu, mu = get_multipole_from_data_set(
                d3, parms, h=h, mtype=mtype, relative=True
            )
            if first:
    ...
    ax.plot([minc, maxc], [sys + rms, sys + rms], 'k--')
    minm, maxm = (
        min([minm, min([sys - rms, sys + rms])]),
        max([maxm, max([sys - rms, sys + rms])]),
    )
    for c in currents:
    ...
    ax.set_ylabel(
        'Relative multipole @ r = ' + str(parms.ref_radius * 1000) + ' mm'
    )
    ax.grid('on')
    ax.legend(loc='best')
    if mtype == 'normal':
...
def plot_magnetic_center(
    ...
    if mtype == 'normal':
        ...
        if save:
            f.savefig('magnetic_center_horizontal_pos.png')
    else:
        if parms.main_multipole_harmonic == 3:
            ...
            ax.set_title('Vertical position where field vanishes')
        if save:
            f.savefig('magnetic_center_vertical_pos.png')
    if show:
        f.show()


def plot_rotation_angle(
    magnet_data_set, parms, currents=None, show=True, save=False, ax=None
):
    if not ax:
        f = _plt.figure()
        ax = _plt.axes()
    ...
    for d1 in magnet_data_set:
        ...
        for d2 in d1:
            ...
            for d4 in d3:
                cu.append(d4.measurement.main_coil_current_avg)
                angle = _math.atan(
                    d4.polynom_a_avg[idx] / d4.polynom_b_avg[idx]
                ) / (parms.main_multipole_harmonic - 1)
                mu.append(1e3 * angle)
            if first:
                ax.plot(cu, mu, _colors[c], label=d4.measurement.magnet_label)
                first = False
    ...
    if save:
        f.savefig('rotation_angle.png')
    if show:
        f.show()


def print_multipoles_single_magnet(
    meas_data_set, parms, current, current_threshold=0.0
):
    pa_avg, pa_std = [], []
    pb_avg, pb_std = [], []
    strheader = '{0:<3s}: {1:^24s}  |  {2:^24s}'.format(
    ...
    for i in range(len(parms.harmonics)):
        ...
        print(
            '{0:02d} : {1:+.3e} +/- {2:.3e}  |  {3:+.3e} +/- {4:.3e}'.format(
                parms.harmonics[i], polyb_avg, polyb_std, polya_avg, polya_std
            )
        )
...
def get_average_excitation_curve(meas_data_set, parms, currents):
    ...
    for current_data_set in meas_data_set:
        exctable, exccurve, tcurrents, harmonics, main_multipole_harmonic = (
            get_excitation_curve(current_data_set, parms)
        )
        multipoles = _numpy.zeros((len(currents), len(harmonics)))
        for i in range(len(harmonics)):
            multipoles[:, i] = _numpy.interp(
                currents,
                tcurrents,
                exctable[:, i],
                left=float('nan'),
    ...
    exctable_std = _numpy.sqrt(
        multipoles_sum2 / len(meas_data_set) - exctable_avg**2
    )
    ...
    return (
        ...
        harmonics,
        parms.main_multipole_harmonic,
    ), (exctable_std, exccurve_std)
    # return currents, multipoles_avg, multipoles_std, harmonics, main_multipole_harmonic


def plot_excitation_curve(
    meas_data_set, parms, currents, show=True, save=False, ax=None
):
    excitation_curve, std = get_average_excitation_curve(
        meas_data_set, parms, currents
    )
    exctable, exccurve, currents, harmonics, main_multipole_harmonic = (
        excitation_curve
    )
    ...
    idx = harmonics.index(main_multipole_harmonic)
    ax.plot(
        currents, exctable[:, idx] - exctable_std[:, idx], '--', color='blue'
    )
    ...
    if save:
        f.savefig('excitation_curve.png')
    if show:
        f.show()


def calc_excitation_curve_nonlinearity(
    meas_data_set, parms, currents, show=True, save=False, ax=None
):
    if not ax:
        f = _plt.figure()
    ...
    fit_error = _numpy.array([
        (pfit(currents[i]) - exctable[i, idx]) / exctable[i, idx]
    ...
    if save:
        f.savefig('excitation_curve_non_linearity.png')
    if show:
        f.show()
...
def plot_hysteresis(
    ...
    if legends:
        ax.legend(legends)
    if save:
        f.savefig('histeresis.png')
    if show:
        f.show()


def plot_trim_coil_excitation_curves(
    meas_data_set,
    parms,
    excitation_curve,
    legends=None,
    ...
    if save:
        f.savefig('trim_coil_excitation.png')
    if show:
        f.show()
