...
import numpy
import string
from math import sqrt,exp,log,pi,asin,atan2,tan,cos,sin,floor
...
# param - list of ring parameters
	...
	#Vectors definition
	phi=numpy.zeros((n_voltas_max+1,h))
	delta=numpy.zeros((n_voltas_max+1,h))
	I0=numpy.zeros(h)
	Vtot=numpy.zeros(h)
	Vharm=numpy.zeros(h)
	Vmain=numpy.zeros(h)
	phimain=numpy.zeros(h)
	phih=numpy.zeros(h)
	phi_final=numpy.zeros(h)
	delta_final=numpy.zeros(h)
	Vb=numpy.zeros((h),dtype=complex)
	Vbn=numpy.zeros((h),dtype=complex)
	VmainC=numpy.zeros((h),dtype=complex)
	...
	for l in range((n_voltas/n_voltas_max)):
		print 'turn # {0:2d} - Dt_max = {1:0.3e}, V3HC = {2:0.2f} kV, Vmain = {3:0.2f} kV'.format(l+1,(phi[n_voltas_max][0]-phi[n_voltas_max][h-1])/w_rf,numpy.average(numpy.absolute(Vbn)),numpy.average(numpy.absolute(Vb)))
		if(l>0):
			for p in range(h):
		...
		for n in range(1,n_voltas_max+1): #loop in turns
			for p in range(h):# loop in bunches
				...
				Vtot[p]=Vg0*numpy.sin(pi-Phi_sync+phi[n][p]+Psi-PhiL)+numpy.real(Vb[p]+Vbn[p])
	...
	Vharm=numpy.absolute(Vbn)
	phih=pi/2+numpy.angle(Vbn)
	...
	Vmain=numpy.absolute(VmainC)
	phimain=pi/2-numpy.angle(VmainC)
