...
from math import sqrt,exp,log,pi,asin,atan2,tan,cos,sin,floor,fabs
...
def Calc_Sigma(Vh,phih,Vmain,phis,I0,nbunches,param):
	...
	sigSh=numpy.zeros(nbunches)
	sigCen=numpy.zeros(nbunches)
	Ratio=numpy.zeros(nbunches)	
	...
	pot=param['ap']/(param['En']*param['C'])*param['cluz']*Vrf/w_rf*(cos(Phi_sync_nat)-numpy.cos(posz/1000*w_rf/param['cluz']+Phi_sync_nat))-param['ap']/(param['En']*param['C'])*U0*posz/1000
	perfil=numpy.exp(-pot/(param['ap']**2*sigma_e**2))
	(pos0,sigma_mm)=calc_sigma(posz,perfil)
	sigma_t=sigma_mm/1000*1/param['cluz']
	perfil=perfil**2
	R0=numpy.trapz(perfil,posz)
	...
	pot=1/(param['En']*param['C'])*param['cluz']/w_rf*(Vrf*(cos(Phi_sync)-numpy.cos(posz/1000*w_rf/param['cluz']+Phi_sync))+Vhideal/m*(cos(m*pi-Phi_synch)-numpy.cos(m*posz/1000*w_rf/param['cluz']+m*pi-Phi_synch)))-1/(param['En']*param['C'])*U0*posz/1000
	perfil=numpy.exp(-pot/(param['ap']*sigma_e**2))
	(pos0,sigma_mm)=calc_sigma(posz,perfil)
	sigma_t=sigma_mm/1000*1/param['cluz']
	perfil=perfil**2
	Rh=numpy.trapz(perfil,posz)
	...
	for i in range(nbunches):
		pot=1/(param['En']*param['C'])*param['cluz']/w_rf*(Vmain[i]*1e3*(cos(Phi_sync_nat-phis[i])-numpy.cos(posz/1000*w_rf/param['cluz']+Phi_sync_nat-phis[i]))+Vh[i]*1e3/m*(cos(m*pi-phih[i])-numpy.cos(m*posz/1000*w_rf/param['cluz']+m*pi-phih[i])))-1/(param['En']*param['C'])*U0*posz/1000
		perfil=numpy.exp(-pot/(param['ap']*sigma_e**2))
		(pos0,sigma_mm)=calc_sigma(posz,perfil)
	...
	print 'Average bunch length = {0:0.2f} +/- {1:0.2f} ps' .format(numpy.mean(sigSh)*1e12,numpy.std(sigSh)*1e12)
	print 'Average bunch length = {0:0.2f} +/- {1:0.2f} mm' .format(numpy.mean(sigSh)*1e3*param['cluz'], numpy.std(sigSh)*1e3*param['cluz'])
	print 'Average bunch length increase = {0:0.2f}' .format(numpy.mean(sigSh)/sigS0)
